# CLAUDE.md — working on hunyuan3d-strix-halo

**Hunyuan3D 2.1 image-to-mesh on AMD Strix Halo (gfx1151), Windows, ROCm —
shape and texture.**
...
1. **Upstream code is never modified.** It is cloned and run as it is, because a
   ...
   hardware needs is injected at launch time**, from `runners/hunyuan3d/shape.py`
...
8. **A replacement for an upstream package is checked against upstream's own
   semantics**, not merely made plausible. The rasterizer returns face indices
   one-based with zero for background, exactly as upstream's CUDA one does, and
   `tests/test_raster.py` checks it that way. **Keep that true of anything new.**
9. **Background removal cannot be skipped.** Without it Hunyuan's
   `recenter()` sets the whole mask to 255, so the subject is never cropped or
   recentred — a silent quality loss rather than an error.
