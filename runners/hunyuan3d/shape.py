...
def blas_backend() -> str:
    ...
    Recorded in metrics because the backends differ per shape (see
    docs/gemm_profile.md), so a timing cannot be interpreted without knowing
    which one produced it. Torch reuses the CUDA names on ROCm; they are
    translated here.
    """
    try:
