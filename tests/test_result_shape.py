...
def _install_stubs() -> Any:
    ...
    **The stubs go in before the import**, because the generating method imports
    ...
    """
    _stub_windows_only("runners.hunyuan3d")

    background = types.ModuleType("runners.hunyuan3d.background")
    background.prepare_image = lambda *a, **k: (_Image(), 0.5)  # type: ignore[attr-defined]
    sys.modules["runners.hunyuan3d.background"] = background

    shape = types.ModuleType("runners.hunyuan3d.shape")
    shape.generate_mesh = lambda *a, **k: _Result()  # type: ignore[attr-defined]
    shape.blas_backend = lambda: "stub"  # type: ignore[attr-defined]
    shape.is_loaded = lambda: False  # type: ignore[attr-defined]
    shape.unload_pipeline = lambda: None  # type: ignore[attr-defined]
    sys.modules["runners.hunyuan3d.shape"] = shape

    from runners.hunyuan3d import __main__ as runner
