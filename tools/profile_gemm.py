...
"""Profile one shape generation and report where the device time goes, GEMM shapes first.
...
wall time. The second runs under ``torch.profiler``, which attributes device
time to operators and collects every GEMM shape (M, N, K, batch, dtype, call
count). Profiling stretches wall time but not the kernels themselves, so
**speed comes from the first run and ratios and per-shape throughput from the
second**.

Unlike the TRELLIS-family runners, the shape pipeline is a single upstream call
with no public stage boundaries, so the profile covers the whole of it (DiT
denoising plus the FlashVDM volume decode) as one stage. The texture stage is
opt-in and is not profiled here.
...
from runners.hunyuan3d import config  # noqa: E402  (config reads dotenv only, no torch)
...
def main() -> int:
    ...
    parser.add_argument("--octree_resolution", type=int, default=None)
    ...
    from runners.hunyuan3d import background, gfxlight, shape
    ...
    try:
        ...
        say("rembg", "removing the background")
        foreground, _fraction = background.prepare_image(Path(args.image), rembg=True)

        say("warmup", "unprofiled generation (honest wall time)")
        warm = shape.generate_mesh(
            foreground,
            steps=args.steps,
        ...
        stage_walls = {"shape": warm.gen_sec}
        say("warmup", f"shape wall {warm.gen_sec:.1f}s")

        # The profiled run repeats the same single upstream call. There is no
        # public stage boundary inside it, so DiT denoising and the volume
        # decode land in one profile together.
        activities = [ProfilerActivity.CPU, ProfilerActivity.CUDA]
        stages: dict[str, dict[str, Any]] = {}

        say("profile", "shape begins")
        with _GemmDtypeRecorder() as rec:
            with profile(activities=activities, record_shapes=True) as prof:
                ...
                torch.cuda.synchronize()
        stages["shape"] = summarize_stage(prof, rec.dtypes, args.top)
        say(
            "profile",
            f"shape: device {stages['shape']['device_total_ms']}ms, "
            f"gemm share {stages['shape']['gemm_share']:.0%}",
        )
    ...
    result: dict[str, Any] = {
        ...
        "params": {
            "steps": warm.steps,
            "octree_resolution": warm.octree_resolution,
            "seed": warm.seed,
        },
