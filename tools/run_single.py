...
Run with the repository's virtual environment:
    ...
    .venv\\Scripts\\python.exe tools\\run_single.py --image assets\\sample.png --out C:\\out ^
        --steps 30 --octree_resolution 384 --seed 1
...
The first run is much slower: MIOpen tunes convolution kernels once. Benchmark
...
def main() -> int:
    ...
    for key in ("steps", "octree_resolution", "guidance_scale", "seed"):
        value = getattr(args, key)
        if value is not None:
            params[key] = value

    proc = subprocess.Popen(
        [sys.executable, "-m", "runners.hunyuan3d"],
        cwd=str(REPO_ROOT),
